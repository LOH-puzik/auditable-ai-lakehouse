...
> **Status:** scaffolded. Detailed content is added as each artefact is implemented.
...
5. **Merkle anchoring** (`src/swift_audit/anchoring/`, `notebooks/07_*.py`) — batch events, build the Merkle tree, anchor the root to Sepolia, emit `AnchorEvent`.
