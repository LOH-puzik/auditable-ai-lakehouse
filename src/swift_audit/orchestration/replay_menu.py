...
from swift_audit.replay.cli import replay_alert
...
def _replay_environment(event: ReplayMenuEvent) -> Iterator[None]:
    values = {
        "SWIFT_AUDIT_GOLD_RECORDS": str(event.gold_records_path),
        "SWIFT_AUDIT_PROMOTION_MANIFEST": str(event.promotion_manifest_path),
        "SWIFT_AUDIT_INFERENCE_EVENTS": str(event.inference_events_path),
        "SWIFT_AUDIT_ANCHOR_BATCH_MANIFEST": str(event.anchor_batch_manifest_path),
    }
