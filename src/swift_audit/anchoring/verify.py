...
from swift_audit.anchoring import AptosLedgerClient, MerkleProof, verify_proof
...
def _find_anchor_manifest(batch_id: str, *, config_path: str | Path) -> Path:
    explicit_manifest = os.getenv("SWIFT_AUDIT_ANCHOR_BATCH_MANIFEST")
    ...
    batches_dir = Path(
        os.getenv("SWIFT_AUDIT_ANCHOR_BATCHES_DIR", str(root / "data/anchor_batches"))
    )
