...
module that emits a `RootAnchored` event containing the 32-byte Merkle root.
"""
...
class AptosLedgerClient(LedgerClient):
    ...
    def event_type(self) -> str:
        ...
        return f"{self.module_address}::{self.module_name}::RootAnchored"
...
def _extract_root_from_transaction(transaction: dict[str, Any], *, event_type: str) -> str:
    ...
    raise ValueError(f"No RootAnchored event found in Aptos transaction for {event_type}")
